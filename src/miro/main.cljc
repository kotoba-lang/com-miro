(ns miro.main
  "Kotodama WASM entrypoint for the Miro clean-room actor (L5) — Clojure port.
  ...
  facts (`miro.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Board"   :plural "boards"   :id-prefix "miro_boa"
    :fields [:name :description :type :viewLink]              :required [:name :description]
    :coerce {}                       :refs {}}
   {:entity "Item"      :plural "items"      :id-prefix "miro_ite"
    :fields [:type] :required [:type]
    :coerce {}          :refs {}}
   {:entity "Tag"     :plural "tags"     :id-prefix "miro_tag"
    :fields [:title :fillColor :type]        :required [:title :fillColor]
    :coerce {} :refs {}}
   {:entity "BoardMember" :plural "boardmembers" :id-prefix "miro_boa"
    :fields [:name :role :type]          :required [:name :role]
    :coerce {}                       :refs {}}
   {:entity "Organization" :plural "organizations" :id-prefix "miro_org"
    :fields [:name :plan :fullLicensesPurchased :type]  :required [:name :plan]
    :coerce {:fullLicensesPurchased :int}          :refs {}}
   {:entity "Connector"    :plural "connectors"    :id-prefix "miro_con"
    :fields [:shape :isSupported :type]  :required [:shape :isSupported]
    :coerce {:isSupported :bool}          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"miro.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "miro-compat" :tier tier :entities entities} 200])
